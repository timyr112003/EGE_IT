...
bunx prisma db push     # создаст/обновит таблицы БД (вкл. журнал ДЗ)
bun run build
...
## 6. Кабинет учителя — журнал сданных ДЗ

Домашнее задание решается прямо на сайте: у каждой задачи — редактор Python,
код сохраняется на странице. Ученик вписывает имя и жмёт «Отправить учителю» —
работа попадает в базу данных сервера (таблица HomeworkSubmission).

Журнал учителя: **http://IP-СЕРВЕРА/uchitelyu?key=КЛЮЧ**

- Ключ задаётся переменной `TEACHER_KEY` в файле `.env` (по умолчанию `urok2026` —
  **обязательно смените** после установки).
- Ссылку с ключом удобно сохранить в закладки — вводить ключ повторно не придётся.
- В журнале: имя ученика, урок, дата, код каждого решения и отметки «решено»;
  есть фильтр по уроку, обновление и удаление работ.
- Запасной канал — кнопка «Word с моими ответами»: ученик присылает .docx вручную
  (например, если сайт временно недоступен).

## 7. Вариант Railway (GitHub → автодеплой)
