...
/*
 * Задачи раздела 4 «Строки: конкатенация и str()».
 * Ученик склеивает строки через + и превращает числа в строки через str().
 */
export const STRING_TASKS: LessonTask[] = [
  {
    id: "str-1",
    shortTitle: "Визитка",
    statement:
      "Вводятся имя и возраст (каждое на своей строке). Выведите две строки: «Меня зовут Имя» и «Мне N лет» — соберите их конкатенацией.",
    hint: "Имя — уже строка, его можно приклеить сразу. Возраст — число, поэтому перед склейкой оберните его в str().",
    solution:
      'name = input()\nage = int(input())\n\nprint("Меня зовут " + name)\nprint("Мне " + str(age) + " лет")',
    starterCode: "# Считайте имя и возраст, соберите визитку через +\n\n",
    tests: [
      { stdin: "Анна\n16", expected: "Меня зовут Анна\nМне 16 лет" },
      { stdin: "Пётр\n20", expected: "Меня зовут Пётр\nМне 20 лет" },
    ],
  },
  {
    id: "str-2",
    shortTitle: "Итого в чеке",
    statement:
      "Вводятся цена товара и количество (два числа). Выведите строку вида «Итого: 75 руб.» — число нужно превратить в строку через str().",
    hint: "Сначала посчитайте стоимость (price * count), затем склейте: \"Итого: \" + str(...) + \" руб.\"",
    solution:
      'price = int(input())\ncount = int(input())\n\nprint("Итого: " + str(price * count) + " руб.")',
    starterCode: "# Цена и количество — выведите итог строкой\n\n",
    tests: [
      { stdin: "25\n3", expected: "Итого: 75 руб." },
      { stdin: "10\n4", expected: "Итого: 40 руб." },
    ],
  },
  {
    id: "str-3",
    shortTitle: "Разделитель",
    statement:
      "Вводятся строка и число n. Выведите строку, повторённую n раз (для разделителя это делает операция *).",
    hint: "Для строк звёздочка означает повтор: \"ха\" * 3 → \"хахаха\". Не забудьте превратить n в число.",
    solution: "s = input()\nn = int(input())\n\nprint(s * n)",
    starterCode: "# Повторите строку n раз\n\n",
    tests: [
      { stdin: "ха\n3", expected: "хахаха" },
      { stdin: "-\n20", expected: "--------------------" },
    ],
